...
file_to_find = "/combined_multiway_registered.ply"
date_to_names = {}
names_to_links = {}
'''
...
def build_public_link(plant_name,plant_date):
    ...
    if link_code == 404:
        print('invalid link')
        print("Name = "+plant_name+" Date = "+plant_date + "Link = "+ test_link)
        names_to_links[plant_name] = 'null'
    else:
        names_to_links[plant_name] = test_link
        #return test_link
#names_to_links = {}
#def buildFast(plant_name, plant_date_list):
    
def build_links_multi(list_names,date):
    names_links = {}
    with ThreadPoolExecutor() as executor:
        for name in list_names:
           executor.submit(build_public_link, name, date)
    print('thread return')

def build_All_links_muti():
    date = getAllDates()
    #could add threading here
    #with ThreadPoolExecutor() as executor:
    for i in date:
        print(i)
        list_names = build_names(i)
        build_links_multi(list_names,i)
        date_to_names[i] = names_to_links.copy()
        names_to_links.clear()
def main():
   build_All_links_muti()
   #tmp = build_map()
   #print(tmp.keys())
   #print(tmp['2020-01-22'])
   #dateL = getAllDates()
   #names = build_names(dateL[0])
   #build_links_multi(names,dateL[0])
   #print(names_links[names[0]])
   print(date_to_names)
